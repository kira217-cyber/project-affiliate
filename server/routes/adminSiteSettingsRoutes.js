...
import AdminSiteSettings from "../models/AdminSiteSettings.js";
import fs from "fs";
...
router.get("/", async (req, res) => {
  try {
    const settings = await AdminSiteSettings.getSettings();
    res.json({
...
router.put("/", uploadAny, async (req, res) => {
  try {
    ...
    if (req.files?.length > 0) {
      ...
      const newFileName = `favicons${ext}`;
      ...
      if (
        settings.favicon &&
        settings.favicon !== "/uploads/method-icons/favicons.png"
      ) {
        const oldPath = path.join(
          process.cwd(),
          "uploads",
          "method-icons",
          path.basename(settings.favicon)
        );
