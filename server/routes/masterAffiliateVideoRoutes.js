...
import MasterAffiliateVideo from "../models/MasterAffiliateVideo.js";
...
router.get("/", async (req, res) => {
  try {
    let data = await MasterAffiliateVideo.findOne();
    if (!data) {
      data = await MasterAffiliateVideo.create({ videos: [] });
    }
...
router.post("/", async (req, res) => {
  try {
    ...
    let data = await MasterAffiliateVideo.findOne();
    if (!data) data = await MasterAffiliateVideo.create({ videos: [] });
...
router.put("/:index", async (req, res) => {
  try {
    ...
    const data = await MasterAffiliateVideo.findOne();
